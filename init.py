...
bl_info = {
    ...
    "description": "Добавляет пункт в Add→Mesh и создаёт треугольную призму с горизонтальным фальцем",
    "category": "Add Mesh"
}
...
def create_triangular_prism_with_horizontal_bevel(
    ...
    # Enter edit mode and select nearly-horizontal edges (along X)
    ...
    for e in bm.edges:
        ...
        # consider edge horizontal if its Z difference is small compared to X/Y span
        if abs(v0.x - v1.x) <= horizontal_x_tol:
            vec = v1.co - v0.co if False else (v1 - v0)  # keep clear intent
            if vec.length > 1e-8:
...
class MESH_OT_add_prism(bpy.types.Operator):
    ...
    bevel_amount: bpy.props.FloatProperty(name="Bevel Amount", default=0.2, min=0.0)
    bevel_segments: bpy.props.IntProperty(name="Bevel Segments", default=3, min=0, max=8)
    enter_editmode: bpy.props.BoolProperty(name="Enter Edit Mode", default=False)
